 ...
 * va FOYDALANUVCHI TANLAGAN TILDA tayyorlanadi (ustunlar, holatlar, sanalar).
 */
export function ExportButtons({ filters = {} }: { filters?: RequestFilters }) {
  ...
  async function handleExport(format: "pdf" | "xlsx") {
    ...
    try {
      ...
      telegram.showAlert(t.export.success(data.count, format.toUpperCase()));
    } catch (err: any) {
      telegram.HapticFeedback.notificationOccurred("error");
      telegram.showAlert(err?.response?.data?.error?.message ?? t.export.failed);
    } finally {
      setPending(null);
    }
  }
