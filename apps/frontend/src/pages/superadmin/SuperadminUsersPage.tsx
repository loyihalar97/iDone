...
import { Card, Spinner, EmptyState } from "@/shared/ui/primitives";
  ...
  return (
    <Card className="mb-3">
      <div className="flex items-center justify-between mb-3">
        <div>
          <p className="font-medium text-tg-text">{user.fullName || "Ism ko'rsatilmagan"}</p>
          <p className="text-xs text-tg-hint">Telegram ID: {user.telegramId}</p>
        </div>
        <span
      ...
      <div className="grid grid-cols-2 gap-2 mb-2">
        <select
          ...
          className="bg-tg-bg border border-black/10 rounded-lg px-2 py-1.5 text-xs text-tg-text"
        >
      ...
      <div className="flex gap-2">
        <button
          ...
          className="flex-1 text-xs py-2 rounded-lg bg-tg-button text-tg-buttonText disabled:opacity-50"
        >
          Saqlash
        </button>
        <button
          ...
          className="flex-1 text-xs py-2 rounded-lg border border-black/10 text-tg-text"
        >
...
export function SuperadminUsersPage() {
  ...
  if (!users || users.length === 0)
    return <EmptyState title="Foydalanuvchilar yo'q" subtitle="Botga /start bosgan foydalanuvchilar shu yerda ko'rinadi" />;

  return (
    <div className="px-4 pt-2 pb-8">
      <p className="text-xs text-tg-hint mb-3 px-1">
